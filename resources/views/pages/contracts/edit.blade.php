        ...
        <div class="col-12 col-lg-6">
            <div class="card shadow-sm h-100">
                <div class="card-header py-2 d-flex justify-content-between align-items-center">
                    ...
                    <span class="badge bg-{{ $contract->status === 'draft' ? 'warning' : 'success' }}">{{ ucfirst($contract->status) }}</span>
                </div>
                    ...
                    <form id="contractForm">
                        ...
                        <div class="row g-2 mb-2">
                            <div class="col-md-6">
                                ...
                                <input type="hidden" id="company_name" name="company_name" value="{{ $contract->company_name }}">
                        ...
                        <div class="row g-2 mb-2">
                            <div class="col-md-4">
                                ...
                                <input type="date" id="effective_date" name="effective_date" class="form-control form-control-sm"
                                       value="{{ $contract->effective_date ? $contract->effective_date->format('Y-m-d') : '' }}">
                            </div>
                            <div class="col-md-4">
                        ...
                        <div class="row g-2 mb-2">
                            <div class="col-md-6">
                                ...
                                <input type="number" id="mou_validity_years" name="mou_validity_years"
                                       ...
                                       value="{{ $contract->mou_validity_years ?? '' }}">
                            </div>
                            <div class="col-md-6">
                                ...
                                <input type="number" id="termination_notice_days" name="termination_notice_days"
                                       ...
                                       value="{{ $contract->termination_notice_days ?? 30 }}">
                            </div>
                        </div>
                        ...
                        <div class="row g-2 mb-2">
                            <div class="col-md-6">
                                ...
                                <textarea id="second_party_description" name="second_party_description"
                                          class="form-control form-control-sm" rows="3">{{ $contract->second_party_description ?? '' }}</textarea>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label mb-1"><strong>Purpose of MOU</strong></label>
                                <textarea id="mou_purpose" name="mou_purpose"
                                          class="form-control form-control-sm" rows="3">{{ $contract->mou_purpose ?? '' }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="mb-2">
                            ...
                            <textarea id="mou_objectives" name="mou_objectives"
                                      class="form-control form-control-sm" rows="2">{{ $contract->mou_objectives ?? '' }}</textarea>
                        </div>
                        ...
                        <div class="row g-2 mb-2">
                            <div class="col-md-6">
                                ...
                                <input type="text" id="vendor_contact_name" name="vendor_contact_name"
                                       class="form-control form-control-sm"
                                       value="{{ $contract->vendor_contact_name ?? '' }}">
                            </div>
                            <div class="col-md-6">
                                ...
                                <input type="email" id="vendor_contact_email" name="vendor_contact_email"
                                       class="form-control form-control-sm"
                                       value="{{ $contract->vendor_contact_email ?? '' }}">
                            </div>
                        </div>
                        ...
                        <div class="mb-2">
                            ...
                            <textarea id="vendor_contact_address" name="vendor_contact_address"
                                      class="form-control form-control-sm" rows="2">{{ $contract->vendor_contact_address ?? '' }}</textarea>
                        </div>
                        ...
                        <div class="d-flex justify-content-end gap-2">
                            ...
                            <button type="button" class="btn btn-info btn-sm" onclick="downloadWord()">
...
function addRow(existingItem = null) {
    ...
    const existingTags = existingItem && existingItem.tags ? existingItem.tags.map(t => t.tag_id || t) : [];
    
    // Tag checkboxes
    let tagCheckboxes = '';
    if (reportingTags && reportingTags.length > 0) {
        reportingTags.forEach(t => {
            const checked = existingTags.includes(t.tag_id) ? 'checked' : '';
            tagCheckboxes += `
    ...
    let tagDisplayText = '-- Select Tags --';
    let tagDisplayClass = '';
    if (existingTags.length > 0 && reportingTags) {
        const selectedNames = reportingTags
            .filter(t => existingTags.includes(t.tag_id))
            .map(t => t.tag_name);
        if (selectedNames.length > 0) {
            tagDisplayText = selectedNames.join(', ');
        ...
        <tr id="row_${rowCounter}">
            ...
            <td>
                <input type="number" class="form-control form-control-sm qty-input" 
                       min="0" step="0.01" placeholder="0" value="${qty}" required>
            </td>
            <td>
            ...
            <td>
                <input type="number" class="form-control form-control-sm rate-input" 
                       min="0" step="0.01" placeholder="0" value="${rate}" required>
            </td>
            <td>
...
function handleSubmit(e) {
    ...
    if (items.length === 0) {
        Toast.error('Please add at least one configuration item.');
        if (!configOpen) toggleConfig();
        return;
    }
    
    if (hasError) {
        Toast.error('Please fill all fields in configuration items.');
        return;
    }

    const contractValue = $('#contractValue').val();
    if (!contractValue || parseFloat(contractValue) <= 0) {
        Toast.error('Please enter contract value.');
        if (!configOpen) toggleConfig();
        return;
    }
    ...
    console.log('Updating:', apiData);

    // Disable button
    $('#saveBtn').prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-1"></span> Updating...');

    // Update contract
    axios.put(`${API_BASE}/${contractId}`, apiData)
        .then(res => {
        ...
        .catch(err => {
            ...
            $('#saveBtn').prop('disabled', false).html('<i class="bi bi-check-lg me-1"></i> Update Contract');
        });
}
</script>
@endpush