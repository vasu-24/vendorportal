...
<head>
    ...
    @endphp
    <title>@yield('title', $organisation->short_name ?? 'Vendor Portal')</title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="csrf-token" content="{{ csrf_token() }}">

    {{-- Dynamic favicon from organisation logo --}}
    @if($organisation && $organisation->logo)
        <link rel="icon" type="image/png" href="{{ asset('storage/' . $organisation->logo) }}">
        <link rel="apple-touch-icon" href="{{ asset('storage/' . $organisation->logo) }}">
        <link rel="shortcut icon" href="{{ asset('storage/' . $organisation->logo) }}">
    @else
    ...
    <footer class="text-center py-3 small mt-auto" 
        ...
        <a href="https://kredo.in" target="_blank"
           style="color: var(--primary-blue); font-weight:600; text-decoration:none;">
            &copy; {{ date('Y') }} {{ $organisation->short_name ?? 'Vendor Portal' }} — Powered by Kredo
        </a>
    </footer>
